...
```
#this is trying to do the yellow-to-red to match ESA/IASI
...
# Merge with the AREA dataframe (this contains density_per_km2)
county_density_map <- merge(
  counties,
  merged_df_area,
  sort = FALSE,
...
county_density_map <- county_density_map[order(county_density_map$order), ]

#summary(county_density_map$density_per_km2)

county_density_map$density_plot <- pmax(
  county_density_map$density_per_km2,
  0
)

#summary(county_density_map$density_plot)

plot_title <- "Animal Density – Post Imputation"

p <- ggplot(county_density_map, aes(long, lat)) +
  geom_polygon(
    aes(group = group, fill = density_plot),
  ...
  scale_fill_gradientn(
    colours = c(
      "#fffffb",  # extra pale yellow
      "#fffff0",  # extra pale yellow
      "#ffffe5",
      "#fff7bc",
      "#fec44f",
      "#fe9929",  # orange
      "#f16913",  # NEW deeper orange
      "#d7301f"   # red
    ),
    ...
    limits = c(0, 200),
    oob = scales::squish,
    breaks = c(0, 0.1, 0.5, 1, 2, 5, 10, 25, 50, 100, 200),
...
plot_title <- gsub("\\.", " ", count_variables["totalCount_initial"]) 
p <- ggplot(county_subset_map, aes(long, lat)) +
  geom_polygon(aes(group = group, fill = get("totalCount_initial")), color = NA, linewidth =0) +
  geom_path(data = states, aes(long, lat, group = group), color = "black") +
  coord_map("albers",  lat0 = 45.5, lat1 = 29.5) +
  labs(title = "Animal Count Filled - Pre Imputation", fill = "Count")  
...
print(p)
```
```{r sanity_check_seams}
ggplot(county_subset_map, aes(long, lat)) +
  geom_polygon(aes(group = group), fill = "grey80", colour = NA) +
  coord_map("albers", lat0 = 45.5, lat1 = 29.5) +
  theme_void()
```
...
```{r}
#sanity check. I think they're seams, not lines
ggplot(county_subset_map, aes(long, lat)) +
  geom_polygon(aes(group = group), fill = "grey80", color = NA) +
  coord_map("albers", lat0 = 45.5, lat1 = 29.5) +
  theme_void()
```
