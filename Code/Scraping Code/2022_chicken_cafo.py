...
def extract_state_data(state_name, fips_code):
    ...
    try:
        ...
        state_layer_count_str = lines[corrected_row_layer_numbers_state_2022].split()[1].replace(",", "")

        if state_layer_count_str != '(D)':
            state_layer_count = int(state_layer_count_str)
        ...
        df = pd.DataFrame({
            ...
            "LAY2022FAC": layer_farms[:min_len],
            "LAY2022NUM": layer_numbers[:min_len],
            "BRL2022FAC": broiler_farms[:min_len],
            "BRL2022NUM": broiler_numbers[:min_len]
        })
...
finalChickenDf.to_csv("../../Data/Scraped Data/2022_cafo/2022_chicken_cafo.csv", index=False)