...
for (i in 1:nrow(state_subset)) {
  ...
  missing <- missing + (state_subset$hog.count[i] - sum(county_subset2$hog.count, na.rm=TRUE)) 
}
...
for (i in 1:length(count_variables)) {
  ...
  p <- p + geom_polygon(data = red_counties, aes(group = group), fill = "white")
...
p <- ggplot(county_subset_map2, aes(long, lat)) +
  ...
  labs(title = "hog Count Filled", fill = "Count")  +
    scale_fill_gradient(name="hogCount",low="white", high="red") 
