---
title: "Linear Data CSV Generator"
...
linear_model_data_sheep <- linear_model_data_sheep[complete.cases(linear_model_data_sheep), ]


# Add state-level totals for area
linear_model_data_sheep <- linear_model_data_sheep %>%
  group_by(state) %>%
  mutate(
    state_total_area     = sum(Area..mi.2., na.rm = TRUE)
  ) %>%
  ungroup()

# List of crop fraction columns to update
crop_cols <- c("corn_fraction", "soy_fraction", "alfalfa_fraction", "total_crop_fraction")
state_crop_cols <- c("state_total_corn", "state_total_alfalfa", "state_total_soy", "state_total_fraction")
...
for (col in state_crop_cols) {
  linear_model_data_sheep[[col]] <- linear_model_data_sheep[[col]] * 0.000247105 * 100 / (linear_model_data_sheep$state_total_area * 640)
}
...
duplicates <- linear_model_data_sheep %>%
  group_by(State, County) %>%
  filter(n() > 1) %>%
  ungroup()
...
linear_model_data_cattle <- linear_model_data_cattle[complete.cases(linear_model_data_cattle), ]
...
# Add state-level totals for area
linear_model_data_broiler <- linear_model_data_broiler %>%
  group_by(state) %>%
  mutate(
    state_total_area     = sum(Area..mi.2., na.rm = TRUE)
  ) %>%
...
for (col in state_crop_cols) {
  linear_model_data_broiler[[col]] <- linear_model_data_broiler[[col]] * 0.000247105 * 100 / (linear_model_data_broiler$state_total_area * 640)
}
...
# Add state-level totals for area
linear_model_data_layer <- linear_model_data_layer %>%
  group_by(state) %>%
  mutate(
    state_total_area     = sum(Area..mi.2., na.rm = TRUE)
  ) %>%
...
for (col in state_crop_cols) {
  linear_model_data_layer[[col]] <- linear_model_data_layer[[col]] * 0.000247105 * 100 / (linear_model_data_layer$state_total_area * 640)
}
...
# Add state-level totals for area
linear_model_data_hog <- linear_model_data_hog %>%
  group_by(state) %>%
  mutate(
    state_total_area     = sum(Area..mi.2., na.rm = TRUE)
  ) %>%
...
for (col in state_crop_cols) {
  linear_model_data_hog[[col]] <- linear_model_data_hog[[col]] * 0.000247105 * 100 / (linear_model_data_hog$state_total_area * 640)
}
