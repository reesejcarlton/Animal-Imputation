...
count_variables <- c("DRY1997NUM", "DRY2002NUM", "DRY2007NUM", "DRY2012NUM", "DRY2017NUM")
...
count_variables <- c("COW2002NUM", "COW2007NUM", "COW2012NUM", "COW2017NUM")
...
count_variables <- c("BRL1997NUM", "BRL2002NUM", "BRL2007NUM", "BRL2012NUM", "BRL2017NUM")
...
county_subset_map <- merge(counties, county_area, sort = FALSE, by.x = c('region', 'subregion'), by.y = c('State', 'County'), all = TRUE)
county_subset_map <- county_subset_map[order(county_subset_map$order), ]

out_dir <- "../Data/AU MAPS/DRY"

#Loop through the different animal names to plot each animal count data

legend_title <- paste("Dairy Cattle per Square Mile 2022")

p <- ggplot(county_subset_map, aes(long, lat)) +
  geom_polygon(aes(group = group, fill = get("dairyCount"))) +
  geom_path(data = states, aes(long, lat, group = group), color = "white", size = 0.03) +
  coord_map("albers",  lat0 = 45.5, lat1 = 29.5) +
  labs(fill = legend_title)  +
  scale_fill_gradient(name=legend_title, limits = c(NA, 60), oob = scales::squish) 


  ggsave(
  filename = file.path(out_dir, paste0("DRY2022NUM", ".png")),
  plot = p,
  width = 10,
  height = 7,
  dpi = 300
...
out_dir <- "../Data/AU MAPS/COW"

#Loop through the different animal names to plot each animal count data

legend_title <- paste("Beef Cattle per Square Mile 2022")
...
p <- ggplot(county_subset_map, aes(long, lat)) +
  ...
  coord_map("albers",  lat0 = 45.5, lat1 = 29.5) +
  labs(fill = legend_title)  +
  scale_fill_gradient(name=legend_title, limits = c(NA, 160), oob = scales::squish) 


  ggsave(
  filename = file.path(out_dir, paste0("COW2022NUM", ".png")),
  plot = p,
  width = 10,
  height = 7,
  dpi = 300
...
out_dir <- "../Data/AU MAPS/BRL"

#Loop through the different animal names to plot each animal count data

legend_title <- paste("Broiler Chicken per Square Mile 2022")
...
p <- ggplot(county_subset_map, aes(long, lat)) +
  ...
  coord_map("albers",  lat0 = 45.5, lat1 = 29.5) +
  labs(fill = legend_title)  +
  scale_fill_gradient(name=legend_title, limits = c(NA, 17000), oob = scales::squish) 


  ggsave(
  filename = file.path(out_dir, paste0("BRL2022NUM", ".png")),
  plot = p,
  width = 10,
  height = 7,
  dpi = 300
...
out_dir <- "../Data/AU MAPS/LAY"

#Loop through the different animal names to plot each animal count data

legend_title <- paste("Layer Chicken per Square Mile 2022")
...
p <- ggplot(county_subset_map, aes(long, lat)) +
  ...
  coord_map("albers",  lat0 = 45.5, lat1 = 29.5) +
  labs(fill = legend_title)  +
  scale_fill_gradient(name=legend_title, limits = c(NA, 8500), oob = scales::squish) 


  ggsave(
  filename = file.path(out_dir, paste0("LAY2022NUM", ".png")),
  plot = p,
  width = 10,
  height = 7,
  dpi = 300
...
out_dir <- "../Data/AU MAPS/HOG"

#Loop through the different animal names to plot each animal count data

legend_title <- paste("Hog per Square Mile 2022")
...
p <- ggplot(county_subset_map, aes(long, lat)) +
  ...
  coord_map("albers",  lat0 = 45.5, lat1 = 29.5) +
  labs(fill = legend_title)  +
  scale_fill_gradient(name=legend_title, limits = c(NA, 800), oob = scales::squish) 


  ggsave(
  filename = file.path(out_dir, paste0("HOG2022NUM", ".png")),
  plot = p,
  width = 10,
  height = 7,
  dpi = 300
...
out_dir <- "../Data/AU MAPS/AU"

#Loop through the different animal names to plot each animal count data

legend_title <- paste("Animal Units per Square Mile 2022")
...
p <- ggplot(county_subset_map, aes(long, lat)) +
  ...
  coord_map("albers",  lat0 = 45.5, lat1 = 29.5) +
  labs(fill = legend_title)  +
  scale_fill_gradient(name=legend_title, limits = c(NA, 140), oob = scales::squish) 


  ggsave(
  filename = file.path(out_dir, paste0("ALL2022AU", ".png")),
  plot = p,
  width = 10,
  height = 7,
  dpi = 300
...
#Loop through the different animal names to plot each animal count data
for (i in 1:length(count_variables)) {
  year <- gsub("DRY|NUM", "", count_variables[i])
  legend_title <- paste("Dairy Cattle per Square Mile", year)
  ...
  p <- ggplot(county_subset_map, aes(long, lat)) +
    ...
    ggsave(
    filename = file.path(out_dir, paste0(count_variables[i], ".png")),
    plot = p,
    width = 10,
    height = 7,
    dpi = 300
...
#Loop through the different animal names to plot each animal count data
for (i in 1:length(count_variables)) {
  year <- gsub("COW|NUM", "", count_variables[i])
  legend_title <- paste("Beef Cattle per Square Mile", year)
...
count_variables <- c("HOG1997NUM", "HOG2002NUM", "HOG2007NUM", "HOG2012NUM", "HOG2017NUM")
out_dir <- "../Data/AU MAPS/HOG"

#Loop through the different animal names to plot each animal count data
for (i in 1:length(count_variables)) {
  year <- gsub("HOG|NUM", "", count_variables[i])
  legend_title <- paste("Hog per Square Mile", year)
...
count_variables <- c("LAY1997NUM", "LAY2002NUM", "LAY2007NUM", "LAY2012NUM", "LAY2017NUM")
out_dir <- "../Data/AU MAPS/LAY"

#Loop through the different animal names to plot each animal count data
for (i in 1:length(count_variables)) {
  year <- gsub("LAY|NUM", "", count_variables[i])
  legend_title <- paste("Layer Chicken per Square Mile", year)
...
#Loop through the different animal names to plot each animal count data
for (i in 1:length(count_variables)) {
  year <- gsub("BRL|NUM", "", count_variables[i])
  legend_title <- paste("Broiler Chicken per Square Mile", year)
...
count_variables <- c("ALL2002AU", "ALL2007AU", "ALL2012AU", "ALL2017AU")
out_dir <- "../Data/AU MAPS/AU"

#Loop through the different animal names to plot each animal count data
for (i in 1:length(count_variables)) {
  year <- gsub("ALL|AU", "", count_variables[i])
  legend_title <- paste("Animal Units per Square Mile", year)
...
```{r}
folders <- c("AU", "COW", "DRY", "LAY", "BRL", "HOG")
base_dir <- "../Data/AU MAPS"

for (f in folders) {
  img_dir <- file.path(base_dir, f)
  
  files <- list.files(img_dir, pattern = "\\.png$", full.names = TRUE)
  files <- sort(files)

  imgs <- image_read(files)
    gif <- image_animate(imgs, fps = 1)  # adjust fps if desired
  
  gif_path <- file.path(img_dir, paste0(f, "_Map.gif"))
  image_write(gif, path = gif_path)
  
  message("Created GIF: ", gif_path)
}
```
