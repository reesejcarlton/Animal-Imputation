...
linear_model_sheep = zeroinfl(sheepCount ~ surroundingFarmDensity + surroundAnimalDensity + farmDensity + stateSheepFarms + stateSheepCount + surroundingPopulationDensity + populationDensity + personalIncome2022 + tavg_JAN + tavg_FEB + tavg_MAR + tavg_APR + tavg_MAY + tavg_JUN + tavg_JUL + tavg_AUG + tavg_SEP + tavg_OCT + tavg_NOV + tavg_DEC + Area..mi.2. + corn_fraction*total_crop + soy_fraction*total_crop + alfalfa_fraction*total_crop + state_total_fraction*state_total_crop,  dist = "poisson",  data = trainData, maxit = 1000)
...
linear_model_hog = zeroinfl(hogCount ~ surroundingFarmDensity + surroundAnimalDensity + farmDensity + stateHogFarms + stateHogCount + surroundingPopulationDensity + populationDensity + personalIncome2022 + tavg_JAN + tavg_FEB + tavg_MAR + tavg_APR + tavg_MAY + tavg_JUN + tavg_JUL + tavg_AUG + tavg_SEP + tavg_OCT + tavg_NOV + tavg_DEC + Area..mi.2. + corn_fraction*total_crop + soy_fraction*total_crop + alfalfa_fraction*total_crop + state_total_fraction*state_total_crop, data = trainData, maxit = 1000)
...
hog_con cat = rbind(training_linear_model_data_hog, predicting_hog_data)
...
#linear_model_broiler = zeroinfl(broilerCount ~ surroundingFarmDensity + surroundAnimalDensity + farmDensity + stateBroilerFarms + stateBroilerCount + surroundingPopulationDensity + populationDensity + personalIncome2022 + tavg_JAN + tavg_FEB + tavg_MAR + tavg_APR + tavg_MAY + tavg_JUN + tavg_JUL + tavg_AUG + tavg_SEP + tavg_OCT + tavg_NOV + tavg_DEC + Area..mi.2. + corn_fraction*total_crop + soy_fraction*total_crop + alfalfa_fraction*total_crop + state_total_fraction*state_total_crop,  data = trainData,  dist = "poisson", maxit = 1000)
...
linear_model_layer = zeroinfl(layerCount ~ surroundingFarmDensity + surroundAnimalDensity + farmDensity + stateLayerFarms + stateLayerCount + surroundingPopulationDensity + populationDensity + personalIncome2022 + tavg_JAN + tavg_FEB + tavg_MAR + tavg_APR + tavg_MAY + tavg_JUN + tavg_JUL + tavg_AUG + tavg_SEP + tavg_OCT + tavg_NOV + tavg_DEC + Area..mi.2. + corn_fraction*total_crop + soy_fraction*total_crop + alfalfa_fraction*total_crop + state_total_fraction*state_total_crop, dist = "poisson", data = training_linear_model_data_layer, maxit = 1000)
...
merged_df <- merged_df %>%
  mutate(
    ...
    weighted_cattle = cattleCount * 1.135  # Cattle count remains unchanged (no weight given)
  )

merged_df$totalCount <- rowSums(
  merged_df[, c("weighted_sheep", "weighted_hog", "weighted_broiler", "weighted_cattle", "weighted_layer")],
  na.rm = TRUE
)
...
p <- ggplot(county_subset_map, aes(long, lat)) +
  ...
  geom_path(data = states, aes(long, lat, group = group), color = "black") +
  coord_map("albers", lat0 = 45.5, lat1 = 29.5) +
  scale_fill_gradient(limits = c(0, 250), oob = scales::squish) +
  labs(title = "Animal Count Filled - Post Imputation", fill = "Count")
...
p <- ggplot(county_subset_map, aes(long, lat)) +
  ...
  geom_path(data = states, aes(long, lat, group = group), color = "black") +
  coord_map("albers", lat0 = 45.5, lat1 = 29.5) +
  scale_fill_gradient(limits = c(0, 400), oob = scales::squish) +
  labs(title = "Animal Count Filled - Pre Imputation", fill = "Count")
